...
router.get('/proposals', async (req, res) => {
    try {
        ...
        const result = await DAOService.getProposals({ state, page, limit });
...
router.post('/proposals/create', authMiddleware, async (req, res) => {
    try {
        ...
        const { title, description, targetContract, callData } = req.body;
...
router.get('/voting-power/:walletAddress', async (req, res) => {
    try {
        const { walletAddress } = req.params;
        const votingPower = await DAOService.getVotingPower(walletAddress);
