...
mongoose.connect('mongodb://localhost:27017/poker', { useNewUrlParser: true, useUnifiedTopology: true })
  .then(async () => {
    ...
    // Delete ALL transactions for this wallet (no filter)
    const deleteResult = await ChipTransaction.deleteMany({});
    console.log(`Deleted ALL ${deleteResult.deletedCount} transactions from database`);
    ...
    console.log(`Total transactions: ${testTxs.length + 1}`);
