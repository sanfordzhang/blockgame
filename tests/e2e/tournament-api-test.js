...
async function runTest() {
    console.log('=== Tournament Single Client Test ===\n');
    ...
    try {
        ...
        // 首先检查页面的基本 HTML 结构
        const htmlCheck = await Runtime.evaluate({
            expression: `
                (function() {
        ...
        console.log(`Button texts: ${htmlCheck.result.value}`);
        
        // 8. 执行 Fold 操作
        console.log('\nStep 8: Clicking Fold button...');
        ...
        // 10. 等待第二手开始
        console.log('\nStep 10: Waiting for second hand to start (3s delay + buffer)...');
        await sleep(6000);
        
        // 11. 检查第二手是否开始
        console.log('\nStep 11: Checking if second hand started...');
            ...
            expression: `
                (function() {
                    ...
                    return {
                        ...
                        hasFold: Array.from(buttons).some(b => b.textContent.toLowerCase().includes('fold'))
                    };
                })()
            `
        });
        
        console.log(`Button count: ${buttonsAfter.result.value?.count}`);
        console.log(`Has Fold button: ${buttonsAfter.result.value?.hasFold}`);
        
        if (buttonsAfter.result.value?.hasFold) {
            ...
            const buttonsHand3 = await Runtime.evaluate({
                expression: `
                    (function() {
                        const buttons = document.querySelectorAll('button');
                        return Array.from(buttons).some(b => b.textContent.toLowerCase().includes('fold'));
                    })()
                `
            });
            
            if (buttonsHand3.result.value) {
                console.log('✅ Third hand also started! Multi-hand tournament is working!');
            } else {
